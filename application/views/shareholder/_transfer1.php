...
if (isset($_POST['submitTransfer']) && $budget_result) {
  ...
  if (($currentDate < $startd) || (date('Y-m-d', strtotime($value_date) - 1) > $endd)) {
    ...
    if (($total_capital_can_transfer == $howmany)) {
      ...
      $query = mysqli_query(
        $conn,
        "INSERT INTO transfer (seller_account,buyer_account,total_transfered_in_birr,transfer_date,value_date,status_of_transfer,year,agreement,transfer_type,maker,full_transfer) 
    ...
    <div class="col-md-6">
      <div class="box-body">
        ...
        ?>
          ...
          <?php } elseif ($_GET['transfer'] == "account") {
          ?>
